import sympy
from cocalc.tokens import Values, TokenType
from cocalc.calc_excepts import ParserException
from cocalc.nodes import *
from cocalc.interpreter import Environment
...
class Parser:
    ...
    def assign_analysis(self):
        ...
        if res_type is VarNode:
            assign_type = 0
        elif res_type is CallNode:
            assign_type = 1

        if (self.current_token is not None) and (self.current_token.type == TokenType.ASSIGNMENT):
    ...
    def extract_func_args(self):
        ...
        while self.current_token is not None and self.current_token.type != TokenType.PAREN and self.current_token.value != Values.PAREN_CLOSE:
            ...
            arg_count += 1
        if self.current_token is None:
            raise ParserException("Parser: Non closing parenthesis")
